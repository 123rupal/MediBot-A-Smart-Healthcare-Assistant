...
def chat_msg():
    ...
    if request.args["message"] == "undefined":
        ...
        current_state = session.get(session_id, -1)
        if current_state == -1:
            ...
            session[session_id] = current_state + 1
            all_result['name'] = user_message
            ...
            if '2' in user_message.lower() or 'check' in user_message.lower():
                username = all_result['name']
                response.append(username+", what's the disease name?")
            ...
            result, data = get_symptoms(user_message)
            if result:
                response.append(f"The symptoms of {user_message} are:")
                for sym in data:
                    response.append(sym.capitalize())
...
def predict_disease_from_symptom(symptom_list):
    # Your implementation for predicting disease from symptom
    user_symptoms = symptom_list
    # Vectorize symptoms using CountVectorizer
    vectorizer = CountVectorizer()
    X = vectorizer.fit_transform(df['Symptoms'])
    ...
    if len(diseases) == 0:
        ...
        return f"<b>{next(iter(diseases))}</b>",next(iter(diseases))
    else:
        return "<b>, ".join(diseases) + "</b>"," or ".join(diseases)

def get_symptoms(disease):
    disease = disease.lower()
    disease_data = Disease.query.filter_by(name=disease).first()
    if disease_data:
        return True, disease_data.symptoms.split(",")
    else:
        return False, "No such disease found!"
